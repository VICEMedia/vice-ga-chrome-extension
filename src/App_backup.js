   ...
   static reorderCustomDimensions(gaEvent, gaConfig){
     if(gaEvent.tid){
       ...
       for(var i in gaEvent){
         if(i.indexOf('cg') > -1){
           if(gaConfig[gaEvent.tid]){
             if(gaConfig[gaEvent.tid].customGroups[i]){
                cglabel = gaConfig[gaEvent.tid].customGroups[i] + " ("+i+")";
             }else {
                cglabel = i.replace('cg','Content Group ');
             }
           }else{
                cglabel = i.replace('cg','Content Group ');
           }
           output[cglabel] = gaEvent[i]
        }
       }
        // Custom Dimensions
        var cdlabel;
       for(var j in gaEvent){
         if(j.indexOf('cd') > -1){
           if(gaConfig[gaEvent.tid]){
                 if(gaConfig[gaEvent.tid].customDimensions[j]){
                    cdlabel = gaConfig[gaEvent.tid].customDimensions[j] + " ("+j+")";
                 }else {
                    cdlabel = j.replace('cd','Custom Dimension ');
                 }
        } else{
              cdlabel = j.replace('cd','Custom Dimension ');
         }
         output[cdlabel] = gaEvent[j];
       }
      }
       // Custom Metrics
       var cmlabel ;
       for(var k in gaEvent){
         if(k.indexOf('cm') > -1){
           if(gaConfig[gaEvent.tid]){
             if(gaConfig[gaEvent.tid].customMetrics[k]){
                cmlabel = gaConfig[gaEvent.tid].customMetrics[k] + " ("+k+")";
             }else {
                cmlabel = k.replace('cm','Custom Metric ');
             }
          } else{
              cmlabel = k.replace('cm','Custom Metric ');
          }
         output[cmlabel] = gaEvent[k];
         }
       }
       return output;
     } else{
